...
for (const { rule, code, points } of [
  { rule: "max attempts", code: "ABCD0007", points: 0 },
  { rule: "point deduction", code: "ABCD0008", points: 80 },
]) {
  test(`record attempts with ${rule} enabled`, async ({ page }) => {
    await page.goto(`/${code}`);
    const problem = page.getByRole("region", {
      name: "Problem 1",
      exact: true,
    ...
    await expect(
      page.locator(".summary").getByText(`${points}p`, { exact: true }),
    ).toBeVisible();
    await page.reload();
    await problem.getByRole("button", { name: "Edit" }).click();
    await expect(problem.getByRole("status", { name: "Attempts" })).toHaveText(
  ...
  test(`failsafe records attempts with ${rule} enabled`, async ({ page }) => {
    await page.goto(`/failsafe/${code}`);
    const problem = page.getByRole("region", {
      name: "Problem 1",
      exact: true,
    });
    ...
    await problem.getByRole("button", { name: "Add failed attempt" }).click();
    await expect(problem.getByRole("status", { name: "Attempts" })).toHaveText(
      "1 attempt",
    );
  });
